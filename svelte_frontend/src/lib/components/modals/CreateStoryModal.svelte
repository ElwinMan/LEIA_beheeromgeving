<script lang="ts">
  ...
  $: selectedBaseLayerName = (() => {
    ...
    const layer = allLayers.find(l => l.id.toString() === baseLayerId || l.id === parseInt(baseLayerId));
    return layer ? layer.title : 'Onbekende laag';
  })();

  $: availableBackgroundLayers = allLayers.filter(layer => layer.isBackground === true);
  $: availableFeatureLayers = allLayers.filter(layer => layer.isBackground !== true);
